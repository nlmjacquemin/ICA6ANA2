...
get_exp_folders <- function(
  input_folder,
  exps
){
  
  exp_folders <- list.files(input_folder)
  
  input_exp_folders <- exp_folders[
    grepl(
      paste(exps, collapse='|'),
      exp_folders
    )
  ...
  return(input_exp_folders)
}

load_abd<-function(
  input_folder,
  exps,
  input_default_folder = "dada/result/intermediary",
  input_default_file = "abd_undechimerized.rds"
){

# input_folder = input_folder
# exps = exps
# input_default_folder = "dada/result/intermediary"
# input_default_file = "abd.rds"

  input_exp_folders <- get_exp_folders(
    input_folder,
    exps
  )
  ...
  for (input_exp_folder in input_exp_folders){
    
    input_file = file.path(
      input_folder,
      input_exp_folder,
      input_default_folder,
      input_default_file 
    )

    if (file.exists(input_file)) {
      
      print(paste0("Loading and merging:",input_file))
      ...
      if (i == 0) {
        ...
        abd <- mergeSequenceTables(abd, abd_one_run)
      ...
      print(paste0("Skipping as no file found for:",input_file))
...
if (is.null(config$experiments)){
  
  exps <- basename(dirname(getwd()))
...
abd <- load_abd(
  input_folder = "../..",
  exps = exps,
  input_default_folder = file.path(
    "dada",
    output_intermediary
    ),
  input_default_file = abd_default_file
)
